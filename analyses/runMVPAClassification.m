...
% Required for bootstrap code to function
...
%             filesep '*.nii']); %subjects{iteration} filesep
  

%JDC edited
       masks = dir([directory.Analysis filesep 'searchlight_mask' filesep  ...
             '*.nii']); %subjects{iteration} filesep 
    ...
    for curMask = 1:length(masks)
        ...
        switch regressRT.flag
            case 'Yes'
                files = dir([directory.Model filesep subject filesep 'Run*']); %studyPath 7/30/21
        ...
        switch analysisType
            case 'Searchlight'
                ...
                % partitioning scheme (cross-validation) and a Support
            ...
            %can determine searchlight size by count or radius in voxels, radius is more common
            
            %searchlightSize=100;
            %metric='count';
            
            %examples, will vary by voxel size and radius****** 
            %searchlightSize=1.5;  %Average 17.5 voxels per light
            %searchlightSize=2.5;  %Average 70.9 voxels per light
            searchlightSize=searchlight.Size;  %Average 20.3 voxels per light
            
            metric = searchlight.Metric;
                ...
                case 'No'
                    for j=1:length(finalPredictions(:,curMask))
                        if isnan(finalPredictions(j,curMask))==1
                            correct.regionName(j,curMask)=NaN;
                        elseif finalPredictions(j,curMask)==currDataset.sa.targets(j)
                            correct.regionName(j,curMask)=1;
                        else
