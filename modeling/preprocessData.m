...
% respectively).
%
% If SPM12 is the preferred processing, the script will batch multiple
        ...
        for csub = 1:length(subjects)
            ...
            for j=1:taskInfo.Runs
                copyMotion = ['cp -p $processDir/$subject/func/run' num2str(j)...
                    ...
                    num2str(j) '_motionRegressors.tsv"']; %'_motion.txt"']%% change to match name/file type from fmriprep output func files
                system(copyMotion);
            end
            ...
            for j=1:length(tsvFiles)
                ...
                filename = strrep(tsvFiles(j).name,...
                 'desc-confounds_regressors.tsv','motionRegressors.txt');
                    % 'motion.txt','motionRegressors.txt');
